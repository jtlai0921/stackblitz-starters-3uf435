...
import { SPEC11070101Req } from './spec11070101-req';
// -- Other Library -- //

@Injectable()
export class SPEC11070101ApiService extends ApiBaseService {
    protected serviceId = 'SPEC11070101'; // API Name
    ...
    getPageData(reqData: object, option?: object, paginator?: object): Promise<any> {
        let sendObj = new ApiRequestOption(SPEC11070101Req, {
            'reqData': reqData,
            'paginator': paginator
        });
        // 欄位檢核
        ...
        return this.send(sendObj, option).then(
            (resObj: ApiResponseOption) => {
                ...
                // 台幣資訊(detail)
                output.data = this._formateService.checkObjectList(jsonObj, 'showData');
                // 台幣總計
                output.totalData = this._formateService.checkObjectList(jsonObj, 'showDataTot');
                // 帳號相關
