...
export class AccountMainComponent implements OnInit {
    ...
    onAcctBackEvent(e) {
        ...
        if (e.hasOwnProperty('data')) {
            ...
            if (change_act == this.acctData.account && change_currency == this.acctData.currencyCode) {
                return false;
            }
            this.acctData.account = change_act;
            this.acctData.currencyCode = change_currency;
        }
        let pageData = {
            search_data: {
    ...
    onChangePage(pageType?: string, pageData?: any) {
        ...
        if (pageType === 'search-box') {
            ...
            this.reqData = {
                ...
                currencyCode: this.acctData['currencyCode']
            };
    ...
    onScrollEvent(next_page) {
        ...
        let componentRef: any = this.paginatorCtrl.addPages(this.nowAppendBox, AccountDetailComponent);
    ...
    private _resetPage() {
        ...
        this._mainService.resetHasYearList(); // 重製列表顯示年月標題資料
        this.pageCounter = 1;
        this.totalPages = 0;
        this.dataTime = '';
