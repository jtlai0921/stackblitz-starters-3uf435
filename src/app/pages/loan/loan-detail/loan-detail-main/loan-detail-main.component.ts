...
@Component({
  selector: 'app-loan-detail-main',
  templateUrl: './loan-detail-main.component.html',
  styleUrls: []
})

export class LoanDetailMainComponent implements OnInit {
  selectTag = 'details'; // 預設本金(畫面顯示),  details:本金利息明細查詢, interest:利息明細查詢
  bookmarkData = []; // 頁籤設定
  searchBoxRule: any;
  ...
  private _initEvent() {
    ...
    this._mainService.removeAllCache('interest');
    // 取得注意資訊
  ...
  /**
   * 切換類別Tag (本金or明細)
   * @param type 1:本金, 2:利息
   */
  onSelectTag(type) {
    if (type == '1') {
      this.selectTag = 'details'; // 切換為本金
    } else {
      this.selectTag = 'interest'; // 切換為利息
    }
    let pageData = {
      search_data: {
    ...
    this._logger.log("into onSelectTag, selectTag:", this.selectTag);
    this._logger.log("into onSelectTag, pageData:", pageData);
    this._resetPage();
  ...
  onAcctBackEvent(e) {
    ...
    this._resetPage();
    this._logger.log("reqData.id:", this._formateServcie.transClone(this.reqData.id));
    this.onChangePage(this.reqData.id, pageData);
  }
  ...
  onBookMarkBack(e) {
    ...
    // 本金
    if (set_data.parent == 'detail') {
      this.selectTag = 'details';
      // 利息
    } else {
  ...
  onScrollEvent(next_page) {
    ...
    if (this.selectTag == 'details') {
      componentRef = this.paginatorCtrl.addPages(this.nowAppendBox, LoanDetailDetailsComponent);
    } else {
      componentRef = this.paginatorCtrl.addPages(this.nowAppendBox, LoanDetailInterestComponent);
    }
