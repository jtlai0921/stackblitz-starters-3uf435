...
export class LoanDetailService {
    ...
    // 本金
    private setDetilCacheName = {
        'list': 'loan-detail-details'
    };
    // 利息
    private setIntCacheName = {
        'list': 'loan-detail-interest'
    };
    ...
    constructor(
        ...
        private spec08020101: SPEC08020101ApiService,
        private spec08020201: SPEC08020201ApiService
    ) {
    }
    ...
    getBookmark() {
        ...
        output.push({
            id: 'detail', // 本金
            name: 'LOAN.LOAN_DETAIL.LOAN_DETAIL_DETAILS',
            sort: 1,
            default: 'toyear',
            data: detail_data
        });
        output.push({
            id: 'interest', // 利息
            name: 'LOAN.LOAN_DETAIL.LOAN_DETAIL_INTEREST',
            sort: 2,
            default: 'toyear',
            data: detail_data
        });
    ...
    removeAllCache(type?: string, acctObj?: object) {
        if (type == 'detail') {
            type = 'loan-detail-details';
        }
        if (type === 'interest') {
            type = 'loan-detail-interest';
        }
     ...
     * @param option 模式設定
     */
    getDetailData(reqData, paginator, option?: object): Promise<any> {
        this._logger.log("into getDetailData, reqData:", reqData);
        this._logger.log("into getDetailData, paginator:", paginator);
        ...
        let cache_main_key = this.setDetilCacheName.list;
        ...
        // 目前僅提供固定排序欄位
        paginator['sortColName'] = 'date';
        // 本金明細查詢
        return this.spec08020101.getPageData(reqData, option, paginator).then(
            (sucessObj) => {
    ...
    getInterestData(reqData, paginator, option?: object): Promise<any> {
        ...
        let cache_main_key = this.setIntCacheName.list; // 設定cache key名稱
        let cache_sub_key = [start_date, end_date, actId, ccy, page, sort];
        const cache_check = this._cacheService.checkPaginatorCach(cache_main_key, page, [], option, null, { sub_key: cache_sub_key });
        const cache_key = cache_check.cache_key;
        if (cache_check.status) {
        ...
        this._logger.log("cache_option:", cache_option);
        // 目前僅提供固定排序欄位
        paginator['sortColName'] = 'intDate';

        // 本金明細查詢
        return this.spec08020201.getPageData(reqData, option, paginator).then(
            (sucessObj) => {
                this._logger.log("send api success, success:", sucessObj);
                this._cacheService.save(cache_key, sucessObj, cache_option);
                return Promise.resolve(sucessObj);
    ...
    private setDateCheck(set_key: string) {
        ...
        console.log("setDateCheck, set_obj:", set_obj);
        console.log("setDateCheck, dateType:", dateType);
        let date_data = this._checkService.getDateSet(set_obj, dateType);
        console.log("setDateCheck, date_data:", date_data);
